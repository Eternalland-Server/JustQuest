...
public class RealmBreakMission extends AbstractMission {

    private final int requirement;

    public RealmBreakMission(String ID, String type, List<String> navigationEvents, List<String> completeEvents, List<String> descriptions, ConfigurationSection section) {
    ...
    public ScreenUI getProgressDisplay(UUID uuid) {
        ...
        ui
                .addComponent(new LabelComp("require", "&f进行境界突破: 0/1")
    ...
    public ScreenUI getCompleteDisplay(UUID uuid) {
        ScreenUI ui = new ScreenUI(QuestUIManager.QUEST_OBJECTIVE_ID);
        ui
                .addComponent(new LabelComp("require", "&f进行境界突破: 1/1")
                        .setExtend("objectives")
                );
        return ui;
    }
